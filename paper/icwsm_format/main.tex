...
\textbf{Regression analysis}   We perform several regression analysis to further explore this relationship. Four models are created with response variables being the logarithm of kudos, hits, comments, and bookmarks respectively. The term novelty and topic novelty scores are the predictor variables in all models. To account for possible non-linear relationships such as the inverse U-shape curve, we also use the squared value of the term novelty and topic novelty as predictor variables. The values are mean-centered to avoid multicollinearity.
...
After controlling for these effects, we find that the term novelty and its square value has coefficients close to zero, indicating little influence on success. The topic novelty has significant negative coefficients in all models. The square value of topic novelty, however, has significant positive coefficients. 


and topic novelty both have significant negative coefficients, supporting our previous observation that higher novelty is linked to less kudos, hits, comments, and bookmarks. 
