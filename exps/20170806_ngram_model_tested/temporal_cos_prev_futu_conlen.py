...
def main(fandom):
    ...
    for i in range(1, len(windows)):
        ...
        if len(df_t_a) > 100 and len(df_t_b) > 100:
            df_t_a = df_t_a.sample(10)  
            df_t_b = df_t_b.sample(10)
    ...
    df_all_prev.to_csv(fandom + '_temporal_cos_toprev_conlen.tsv', index = False, sep = '\t')

    df_all_futu = pd.concat(df_all_futu)
    df_all_futu.to_csv(fandom + '_temporal_cos_tofutu_conlen.tsv', index = False, sep = '\t')
