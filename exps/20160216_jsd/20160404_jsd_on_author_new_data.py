...
for month in timelist[42:]:
    ...
    if len(text) > 0:
        ...
        with open('jsds_on_author_new_data.txt', 'a') as g:
            g.write(str(month) + '\t' + str(monthly_ave_resampled_jsds))
            g.write('\n')

        with open('author_count_new_data.txt', 'a') as g2:
            g2.write(str(author_count))
            g2.write('\n')
