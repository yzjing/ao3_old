...
def write_header(outfile):
    ...
    keys = ['AdditionalTags', 'ArchiveWarnings', 'Author', 'Bookmarks', 'Category', 'ChapterIndex', 'Chapters', 'Characters', 'Comments', 'CompleteDate', 'Fandoms', 'Hits', 'Kudos', 'Language', 'Notes', 'PublishDate', 'Rating', 'Relationship', 'Summary', 'Text', 'Title',  'URL', 'UpdateDate','Words']
...
#     get work metadata and contents into a dictionary.
    ...
    try:
        ...
        work['Fandoms'] = [i for i in fandom if i != ''] if fandom != [] else ''
...
for i in range(1, <fandom_max_page>, 100):
    start_page = i
    max_page = i + 100 - 1
    scr_insert = '''
outfile = './<fandom_name>_%s_%s.csv'
logfile = './log_%s_%s'
max_page = %s
try:
    ...
    start_page = %s
    '''%(start_page, max_page, start_page, max_page, max_page, start_page)
    with open('scraper_%s_%s.py'%(start_page, max_page), 'w') as g:
        g.write(scr1 + scr_insert + scr2)
