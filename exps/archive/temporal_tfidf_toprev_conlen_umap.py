    ...
    for i in range(1, len(windows)):
        if int(windows[i][0][:4]) > 2012:
            ...
            if len(df_t_curr) > 50 and len(df_t_prev) > 100:
                ...
                vectorized_prev = vectorizer.transform(df_t_prev.Text.tolist()).todense()
                embedding = umap.UMAP(n_neighbors=10).fit_transform(vectorized_prev)
                fname = fandom + '_' + windows[i][0] + '.p'
