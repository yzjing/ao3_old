...
def main(fandom):
    ...
    for i in range(1, len(windows)):
        ...
        if len(df_t) > 100 :
            ...
            df_t['Text'] = df_t.apply(lambda row:sample_words(row['Text']), axis=1)
            ...
            df_t['Cos'] = df_t.apply(lambda row: spatial.distance.cosine(row['Dist'], std), axis=1)
            del df_t['Dist']
            df_all.append(df_t)

            
    df_all = pd.concat(df_all)
    df_all.to_csv(fandom + '_temporal_tfidf_cos_tocurr_samplew.tsv', index = False, sep = '\t')
