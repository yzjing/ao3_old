...
def main(fandom):
    ...
    for time in timelist:
        ...
        if len(prev_window) > 0:
            ...
            if len(df_t_curr) > 20 and len(df_t_prev) > 50:
                ...
                prev_longdoc = ' '.join([line for line in df_t_prev.Text.tolist()])
                prev_std = vectorizer.transform([prev_longdoc]).toarray()
    ...
    df_all.to_csv(fandom + '_temporal_tfidf_cos_toprev_conlen_longcent.tsv', index = False, sep = '\t')
