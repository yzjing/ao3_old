...
def main(fandom):
    ...
    for i in range(1, len(windows)):
        ...
        if len(df_t_curr) > 50 and len(df_t_prev) > 50:
            if len(df_t_curr) > 1000:
                df_t_curr = df_t_curr.sample(1000)
            if len(df_t_prev) > 1000:
                df_t_prev = df_t_prev.sample(1000)
            ...
            k = '_'.join([item for item in windows[i-1]])
            tsne_dict[k] = tsne
    pickle.dump(tsne_dict, open(fandom + 'temporal_tsne_sample.p', 'wb'))
