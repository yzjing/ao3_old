...
def create_timelist(df):
    ...
    timelist = [time for time in timelist if int(time[0:4]) >= 2010 and time != '2016-06']
...
def main(fandom):
    ...
    for i in range(1, len(windows)):
        ...
        if len(df_t_a) > 100 and len(df_t_b) > 100:
            ...
            df_two['Text'] = df_two.apply(lambda row:sample_words(row['Text']), axis=1)
    ...
    df_all_prev.to_csv(fandom + '_temporal_cos_toprev_samplew.tsv', index = False, sep = '\t')

    df_all_futu = pd.concat(df_all_futu)
    df_all_futu.to_csv(fandom + '_temporal_cos_tofutu_samplew.tsv', index = False, sep = '\t')
