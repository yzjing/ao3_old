...
def main(fandom):
    ...
    for i in range(1, len(windows)):
        ...
        if len(df_t_a) > 100 and len(df_t_b) > 100:
            ...
            df_t_a['KL'] = df_t_a.apply(lambda row: calc_kl(row['Dist'], std_b), axis=1)
            del df_t_a['Dist']
            df_all_prev.append(df_t_a)

            df_t_b['KL'] = df_t_b.apply(lambda row: calc_kl(row['Dist'], std_a), axis=1)
            del df_t_b['Dist']
            df_all_futu.append(df_t_b)

    df_all_prev = pd.concat(df_all_prev)
    df_all_prev.to_csv(fandom + '_temporal_kl_toprev.tsv', index = False, sep = '\t')

    df_all_futu = pd.concat(df_all_futu)
    df_all_futu.to_csv(fandom + '_temporal_kl_tofutu.tsv', index = False, sep = '\t')
